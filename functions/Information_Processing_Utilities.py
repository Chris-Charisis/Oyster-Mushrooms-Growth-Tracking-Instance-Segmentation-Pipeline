    ...
    for j,poly in enumerate(polygons[-1]):
        ...
        if len(poly) > 1:
            ...
            if cluster_sizing_option:
                ...
                relative_cluster_area = pixel_relative_area(Polygon(poly).area,averaged_width_pixels,averaged_height_pixels,substrate_real_width,substrate_real_height)
                
                #Updating cluster segments
                cluster_segments = ([img_num+1,j,Polygon(poly).area,relative_cluster_area,polygons_info[-1][j]])
            ...
            cv2.polylines(image_for_visualization,np.int32([poly]),True,(255, 0, 0),10)
            cv2.putText(image_for_visualization,'{}'.format(j),(int(centre.x),int(centre.y)),cv2.FONT_HERSHEY_COMPLEX,4,(0,255,0),6,cv2.LINE_AA)
...
def plot_area_growth_curves(growth_monitoring_file,label_mapping,label_colors,time_interval,show_plots=False):
    # Load generated growth monitoring size information file
    growth_data = pd.read_csv(growth_monitoring_file)
    ...
    plt.tight_layout()
    plt.savefig("./results/individual_relative_cluster_area.png", dpi=300)
    if show_plots:
        plt.show()
